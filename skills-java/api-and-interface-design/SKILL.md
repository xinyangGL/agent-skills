...
public interface TaskApi {
     ...
     * 返回匹配过滤器的分页任务
     */
    PaginatedResult<Task> listTasks(ListTasksParams params);
...
public record Error(
    String code,       // 机器可读："VALIDATION_ERROR"
    String message,    // 人类可读："邮箱是必需的"
    Object details     // 有帮助时的额外上下文
) {}
...
public ResponseEntity<Task> createTask(@Valid @RequestBody CreateTaskInput input) {
...
public record CreateTaskInput(
    ...
    @Null String description,      // 后来添加，可选
    @Null String priority,          // 后来添加，可选
    @Null List<String> labels       // 后来添加，可选
) {}

// 差：更改现有字段类型或移除字段
// public record CreateTaskInput(String title); // 移除字段——破坏现有消费者
```
...
// 消费者获得类型收窄
public String getStatusLabel(TaskStatus status) {
    return switch (status) {
        case Pending p -> "待处理";
        case InProgress i -> "进行中 (" + i.assignee() + ")";
        case Completed c -> "完成于 " + c.completedAt();
