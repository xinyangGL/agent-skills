 ...
 *
 * @param input 任务创建数据
 * @return 带有服务器生成 ID 和日期的已创建任务
 * @throws ValidationException 如果 title 为空或超过 200 字符
 * @throws DatabaseException 如果数据库插入失败
 *
 * @example
 * <pre>{@code
 * Task task = taskService.createTask(new CreateTaskInput(
 *     "完成任务", "完成项目文档", "high"
 * ));
 * System.out.println(task.getId()); // "task_abc123"
 * }</pre>
 */
