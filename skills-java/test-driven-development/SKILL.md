...
// 好：一个断言，一个概念
@Test
void rejectsTasksWithEmptyTitles() {
    var result = taskService.createTask(new TaskRequest("", null, null));
    assertThat(result.getIfPresent().getError().getCode())
        .isEqualTo("INVALID_TITLE");
}

// 避免：多个概念，像在生产中一样编排
@Test
void handlesFullCreationFlow() {
    var user = userService.createUser(...);
    var auth = authService.authenticate(user);
    var task = taskService.createTask(new TaskRequest("Test"), auth);
    assertThat(task.getId()).isNotNull();
    assertThat(task.getTitle()).isEqualTo("Test");
    assertThat(task.getStatus()).isEqualTo("PENDING");
    var updated = taskService.updateTask(task.getId(), new TaskUpdate("done"), auth);
...
// 好：在数据库边界处 Mock
@Test
void returnsTasksFromTheDatabase() {
    when(taskRepository.findAll()).thenReturn(List.of(new Task(1L, "Test")));
...
void marksTaskAsDone() {
    var tasks = taskTracker.add(new TaskRequest("Test"));
    assertThat(tasks.get(0).getStatus()).isEqualTo("PENDING");
    taskTracker.complete(tasks.get(0).getId());
    assertThat(tasks.get(0).getStatus()).isEqualTo("DONE");
}

@Test
void cannotCompleteTaskTwice() {
    var tasks = taskTracker.add(new TaskRequest("Test"));
    taskTracker.complete(tasks.get(0).getId());
    taskTracker.complete(tasks.get(0).getId());
    assertThat(tasks.get(0).getStatus()).isEqualTo("DONE");
...
使用 Selenium 或 Playwright 进行真实的运行时浏览器测试——验证应用程序在浏览器中实际运行。

```java
// 浏览器测试示例——使用 Playwright for Java 运行
@Test
void taskCreationFlowWorksInBrowser() throws Exception {
    browserContext.newPage().navigate("http://localhost:3000/tasks");
    browserContext.newPage().click("[data-testid='new-task-button']");
    browserContext.newPage().fill("[data-testid='task-title']", "Test task");
    browserContext.newPage().click("[data-testid='submit-button']");
    assertThat(browserContext.newPage().locator("[data-testid='task-list']").textContent())
        .contains("Test task");
}
```
...
| "这个方法是私有的，所以我不测试它" | 私有逻辑仍然需要验证。通过公共接口测试它，或直接测试它。 |
