...
```python
# 先定义契约（使用 Pydantic 或 dataclass）
from pydantic import BaseModel, Field

class CreateTaskInput(BaseModel):
    title: str = Field(..., min_length=1, max_length=200)
    description: str | None = Field(None, max_length=1000)
    priority: "low" | "medium" | "high" | None = None
...
class TaskApi(ABC):
    ...
    @abstractmethod
    def update_task(self, id: str, input: UpdateTaskInput) -> Task:
        """部分更新——只有提供的字段改变"""
        ...
...
class Error(BaseModel):
    code: str        # 机器可读："VALIDATION_ERROR"
    message: str     # 人类可读："邮箱是必需的"
    details: dict | None = None  # 有帮助时的额外上下文
...
**不要混合模式。** 如果一些端点抛出异常，其他返回 None，还有一些返回 `{ "error" }`——消费者无法预测行为。
...
```python
# 在 API 边界处验证（FastAPI 自动验证 Pydantic 模型）
@app.post("/api/tasks", status_code=201)
async def create_task(input: CreateTaskInput):
    # FastAPI 自动验证输入
    # 验证后，内部代码信任类型
    task = task_service.create(input)
    return task
```
...
class CreateTaskInput(BaseModel):
    title: str = Field(..., min_length=1, max_length=200)
    description: str | None = None      # 后来添加，可选
    priority: str | None = None         # 后来添加，可选
    labels: list[str] | None = None     # 后来添加，可选

# 差：更改现有字段类型或移除字段
# class CreateTaskInput(BaseModel):
#     title: str  # 移除字段——破坏现有消费者
```
...
| 查询参数 | snake_case | `?sort_by=created_at&page_size=20` |
| 响应字段 | snake_case | `{ "created_at": "...", "updated_at": "..." }` |
| 布尔字段 | is/has/can 前缀 | `is_complete`, `has_attachments` |
...
```python
# 请求
GET /api/tasks?page=1&page_size=20&sort_by=created_at&sort_order=desc
...
```
GET /api/tasks?status=in_progress&assignee=user123&created_after=2025-01-01
```
...
class CompletedStatus(TypedDict):
    type: Literal["completed"]
    completed_at: datetime
    completed_by: str

class CancelledStatus(TypedDict):
    type: Literal["cancelled"]
    reason: str
    cancelled_at: datetime

TaskStatus = PendingStatus | InProgressStatus | CompletedStatus | CancelledStatus
...
def get_status_label(status: TaskStatus) -> str:
    match status["type"]:
        case "pending":
            return "待处理"
        case "in_progress":
            return f"进行中 ({status['assignee']})"
        case "completed":
            return f"完成于 {status['completed_at']}"
        case "cancelled":
            return f"已取消: {status['reason']}"
```
