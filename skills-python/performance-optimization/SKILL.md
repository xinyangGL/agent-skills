...
def create_task(request: TaskRequest) -> Task:
    ...
    duration = (time.perf_counter() - start) * 1000  # 毫秒
    logger.debug("create_task 耗时: %.2fms", duration)
    return task
```
...
def after_cursor_execute(conn, cursor, statement, parameters, context, executemany):
    ...
    logger.debug("查询耗时: %.2fms", total * 1000)
```
...
| 内存增长 | 泄漏的引用、无界缓存、大负载 | 内存 profiling |
...
for task in tasks:
    task.owner = user_repository.find_by_id(task.owner_id)

# 好：单个查询带 JOIN
tasks = task_repository.find_all_with_owner()
```
...
def get_app_config() -> AppConfig:
    return config_repository.find_first_by_type("GLOBAL")

# 或使用带 TTL 的缓存
class Cache:
    def __init__(self, ttl=300):  # 5 分钟
        self.ttl = ttl
        self._cache: dict = {}
        self._expiry: dict = {}
