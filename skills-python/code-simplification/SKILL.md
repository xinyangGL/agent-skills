...
会话没有过期且请求来自受信任的域且...
"""
def authenticate_user(user: User, session: Session, request: Request) -> bool:
    # 50 行复杂的逻辑
    ...

# 考虑将其拆分为更清晰的函数：
def is_authentication_required(user: User, request: Request) -> bool: ...
def is_valid_session(session: Session) -> bool: ...
def is_trusted_domain(request: Request) -> bool: ...

def authenticate_user(user: User, session: Session, request: Request) -> bool:
    if not is_authentication_required(user, request):
        return False
    if not is_valid_session(session):
        raise SessionExpiredError()
    if not is_trusted_domain(request):
...
def process_order(request: OrderRequest) -> Order:
    ...
    # 应用折扣
    if request.discount:
        total *= (1 - request.discount)
    # 保存
    return order_repository.save(Order(user_id=request.user_id, total=total))

# 之后：提取的函数
def validate_order(request: OrderRequest) -> None: ...
def calculate_total(request: OrderRequest) -> float: ...
def apply_discount(total: float, discount: float | None) -> float: ...

def process_order(request: OrderRequest) -> Order:
    validate_order(request)
    total = apply_discount(calculate_total(request), request.discount)
    return order_repository.save(Order(user_id=request.user_id, total=total))
```
